#functions
existsDiff <- function(x, y, xTitle, yTitle, context, commnity) {
  d1 <- as.numeric(x)
  d2 <- as.numeric(y)
  method = NULL
  pvPrint= NULL
  ...
  if(isGaussionDistribution(d1) == FALSE || isGaussionDistribution(d2) == FALSE || isGaussionDistribution(c(d1,d2)) == FALSE) {
    ...
    print(pvPrint)
    print(effectSize)
    result = as.character(effectSize)
    ...
    if(mean(d1) > mean(d2)) {
      ...
      #result = "Effect size in favor d1"
      result = "Em favor de d1"
    } else {
      print("Effect size in favor d2")  
      #result = "Effect size in favor d2"
      result = "Em favor de d2"
    }
    print("--------------------------")
  }
 # print(yTitle)
